...
import { AlertTriangle, Bot, Bug, CheckCircle2, Copy, Database, Cpu, FileText, Files, Folder, FolderInput, Maximize2, PanelRight, PanelLeftClose, PanelLeftOpen, Paintbrush, Pencil, Pin, Plus, RefreshCw, Save, Send, Settings2, Sigma, Sparkles, TestTube2, Trash2, UploadCloud, X } from "lucide-react";
...
function NavigationItem({ active = false, chat = false, detail, icon, label, onCreateChat, onDelete, onEditCancel, onEditChange, onEditSave, onMove, onOpen, onPin, onRename, pinned = false, editing = false, editValue = "" }) {
    ...
    const openActionMenu = (element) => {
        ...
        setMenuAnchor({ top: rect.top, left: rect.right + 2 });
    };
    const closeActionMenu = (relatedTarget) => {
        ...
        if (relatedTarget instanceof Element && relatedTarget.closest(".nav-row")) {
            cancelCloseActionMenu();
            setMenuAnchor(null);
            return;
        }
